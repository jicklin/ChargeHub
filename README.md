...
- 支持使用 InfiniCLOUD WebDAV 作为设备库和资料库同步载体
...
设备数据以 JSON 形式保存；资料库使用“元数据 JSON + 图片文件夹”的形式保存。
...
- 导入设备库 JSON
- 通过系统分享面板快速分享设备库
- 配置 InfiniCLOUD WebDAV 文件夹作为同步载体
- 上传当前设备库与资料库到 InfiniCLOUD
- 从 InfiniCLOUD 合并下载或覆盖本机数据

手动导入和 InfiniCLOUD 下载都支持两种模式：

- 替换当前数据
- 按数据 ID 合并导入 / 下载

InfiniCLOUD 同步会在你配置的 WebDAV 文件夹中保存：

- `devices.json`：设备库
- `reference-photos.json`：资料库元数据，不再内嵌图片二进制 / Base64
- `reference-images/`：资料库图片文件夹，图片按独立文件保存

建议为 ChargeHub 创建单独的远端目录，例如：

```text
https://你的账号.infini-cloud.net/dav/ChargeHub/
```

实际地址以你的 InfiniCLOUD WebDAV 设置页为准；密码建议使用 InfiniCLOUD 提供的应用密码或 WebDAV 专用密码。
